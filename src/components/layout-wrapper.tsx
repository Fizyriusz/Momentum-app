...
function MobileSidebarNav({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <Link href="/" onClick={onClose} className="p-6 flex items-center gap-3 border-b border-zinc-800/50 shrink-0 hover:opacity-80 transition-opacity">
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-zinc-800">
        <div>
          ...
          <ul className="space-y-1">
            <li>
              ...
              >
                ...
                {todayCount > 0 && <span className="text-xs font-bold text-purple-400">{todayCount}</span>}
              </Link>
            </li>
            ...
            <li>
              <Link
                href="/tasks/tomorrow"
                onClick={onClose}
                className={`flex items-center justify-between px-3 py-2 rounded-xl transition-all font-medium text-sm ${pathname === "/tasks/tomorrow" ? "bg-purple-500/10 text-purple-400 font-bold" : "text-zinc-400 hover:text-zinc-200"}`}
              >
                <div className="flex items-center gap-3">
                ...
                projects.map(project => {
                  ...
                  return (
                    <Link
                      ...
                      onClick={onClose}
                      className={`flex items-center gap-3 px-3 py-2 rounded-xl transition-all duration-200 font-medium text-sm ${isActive ? "bg-purple-500/10 text-purple-400 font-bold" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/50"}`}
                    >
...
export function LayoutWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-zinc-950 overflow-hidden text-zinc-100 selection:bg-purple-500/30">
      ...
      <aside className="w-64 border-r border-zinc-800 shrink-0 hidden md:block">
        <Suspense fallback={<div className="w-64 h-full bg-zinc-950" />}>
